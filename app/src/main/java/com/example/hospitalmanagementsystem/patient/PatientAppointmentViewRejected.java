...
public class PatientAppointmentViewRejected extends AppCompatActivity {

    String userID;
    RecyclerView viewAppointmentReject;
    String doctorID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_patient_appointment_view_rejected);

        userID = getIntent().getExtras().get("userID").toString();

        loadingBar = new ProgressDialog(this);

        viewAppointmentReject = findViewById(R.id.viewAppointmentReject);
        viewAppointmentReject.setHasFixedSize(true);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this);
        viewAppointmentReject.setLayoutManager(linearLayoutManager);
    ...
    private void startListen() {
        Query query = FirebaseDatabase.getInstance().getReference().child("Users").child(userID).child("AppointmentRejected").limitToLast(50);
        ...
        viewAppointmentReject.setAdapter(adapter);
